...
#### 快速移动 (`flash.nvim`)
*   **`f` + `[字符]`**: 按 `f` 后，输入任意字符，即可在屏幕上所有匹配项之间快速跳转。
...
#### 窗口交互
*   在终端的输入模式下，按 **`<Esc>`** 或 **`jk`** 可以退回到普通模式。
*   在终端的普通模式下，按 **`Ctrl` + `h/j/k/l`** 可以将光标焦点快速切换到相邻的 Neovim 窗口。

#### 如何强制关闭死机的终端
如果终端内的程序卡死，导致您无法输入 `exit`：
1.  首先，在终端内按 **`Ctrl` + `h/j/k/l`** 中的任意一个，将焦点“逃离”到旁边的代码窗口。
2.  然后，用 **`Ctrl+w`** + **`h/j/k/l`** 将光标移到那个死机的终端窗口上（此时您在普通模式）。
3.  最后，按下我们配置好的关闭缓冲区快捷键 **`<leader>bd`**，即可强制关闭终端。
