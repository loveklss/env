#!/usr/bin/env python3
"""Print wiki session info. Uses same session file as local_mcps (~/.cursor/wiki_sessions.json)."""
...
def session_info(url: str) -> str:
    ...
    if not session:
        return f"No session found for {domain}.\nTo login: python -m local_mcps.cli wiki_login"
    try:
...
def main():
    ...
    if not url:
        print("Usage: wiki_session_info.py <url>", file=sys.stderr)
        sys.exit(1)
